...
- **跨会话记忆**——基于 Mem0 的人级长期记忆：所有项目与机器在 `feishu:<身份>` 下共用一个固定桶；密钥与原始工具输出不进入捕获。
...
直接依赖 `mem0ai` 钉在 3.0.8（首个使用 `uuid` 11.1.1 的兼容 3.x 版本）；已安装生产树的 `npm audit --omit=dev` 干净。Mem0 自动捕获用户消息和助手文本，在配置的 `feishu:<身份>` 下写入一个与路径无关的固定 `feishu` 桶，记忆随人跨项目、跨机器延续（会话、Skills、包和设置仍按项目隔离）。原始工具输出不自动捕获；全局记忆需要显式动作。配置的 `feishu:<身份>` 覆盖外部 `MEM0_USER_ID`，`MEM0_API_KEY` 仅环境变量，遥测关闭。启动时做有界健康检查；recall、capture 或 Dream 失败时同时在终端和交互界面发出警告，并在该降级会话中禁用后续记忆动作。之后一次健康的调用会恢复，且不改动无关的 Feishu 设置。
...
- 两个项目共享配置的 `feishu:<身份>` 及其唯一固定 Mem0 桶，而会话、私有 Skills、包设置、已加载包 Skills 彼此独立；
