...
test("help exposes only the Feishu Agent surface and security boundary", () => {
  ...
  for (const text of ["feishu -p", "feishu init", "feishu install", "feishu remove", "feishu list", "feishu update", "feishu config", "feishu skills sync", "/find-skill", "/remote", "-c", "-r", "--session", "--lark-profile"]) assert.match(result.stdout, new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
