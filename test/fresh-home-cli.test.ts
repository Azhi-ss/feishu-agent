...
test("fresh HOME CLI init is idempotent and immediately ready for Print without project .pi or secret leakage", async () => {
  ...
  writeFileSync(join(bin, "fake-npm"), `#!/bin/sh\nset -eu\nif [ "$1" = install ]; then\n  prefix=""\n  while [ "$#" -gt 0 ]; do [ "$1" = --prefix ] && { prefix="$2"; break; }; shift; done\n  mkdir -p "$prefix/node_modules/@mem0" "$prefix/node_modules/@azhi-ss"\n  ln -sfn "${join(repoRoot, "node_modules", "@mem0", "pi-agent-plugin")}" "$prefix/node_modules/@mem0/pi-agent-plugin"\n  ln -sfn "${join(repoRoot, "packages", "feishu-remote")}" "$prefix/node_modules/@azhi-ss/feishu-remote"\n  printf '{"dependencies":{"@mem0/pi-agent-plugin":"0.1.5","@azhi-ss/feishu-remote":"0.1.0"}}' > "$prefix/package.json"\n  exit 0\nfi\nexit 0\n`, { mode: 0o755 });
