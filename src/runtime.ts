...
export async function createRuntime(cwd: string, projectRoot: string, projectKey: string, agentHome: string, resume = false, currentRequest?: string, selectSession = false) {
  ...
  const resourceLoader = new FeishuResourceLoader(agentHome, projectRoot, projectKey, currentRequest, memory.extension, selectSession);
  ...
  return createAgentSessionRuntime(createSession, { cwd, agentDir: agentHome, sessionManager: selected.manager });
}
